...
const Login = () => {
  ...
  return (
    <div>
        ...
        <img className='w-[100vw] h-[100vh]' src={img1} alt='backgroundimg'/>  
        </div>
        <form onSubmit={getInputData} className='flex flex-col w-3/12 p-12 my-36 left-0 right-0 mx-auto items-center absolute bg-black opacity-90 rounded-md'>
          ...
          <div className='flex flex-col'>
            ...
            <p className='text-white mt-2'>{isLogin ? "New to Eclipsio? " :"Already have an account? "}<span onClick={loginHandler} className='ml-1 text-blue-900 font-medium cursor-pointer'>{isLogin? "Signup":"Login"}</span></p>
          </div>
        </form>
    </div>
  )
